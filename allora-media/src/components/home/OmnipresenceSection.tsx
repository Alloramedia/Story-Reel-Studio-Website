...
export function OmnipresenceSection() {
  return (
    <Section variant="dark">
      ...
      <RadialGlow color="#65B207" position="20% 40%" size="600px" opacity={0.035} />
      <div className="mx-auto max-w-5xl py-6">
        <SectionHeader
          eyebrow="The Omnipresence Advantage"
          title={<>Show up everywhere.{' '}<span className="text-[#65B207]">Get chosen every time.</span></>}
        ...
        >
          <h3 className="mb-6 text-center font-heading text-xl font-bold text-white">
            How Allora builds your omnipresent brand
          </h3>
          <div className="grid gap-6 md:grid-cols-2">
            {omnipresenceSteps.map((item, i) => (
              <div key={i} className="flex items-start gap-4">
                <span className="font-heading text-3xl font-black text-[#65B207]/20">{item.step}</span>
                <div>
          ...
          <div className="mt-8 rounded-xl bg-[#131E13] p-5 text-center">
            <p className="text-sm text-white/60">
              <span className="font-bold text-[#65B207]">Omnipresence isn&apos;t about being loud — it&apos;s about being everywhere that matters.</span>{" "}
